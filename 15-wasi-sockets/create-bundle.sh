...
# wasi-sdk. Produces the layout described in DESIGN.md:
#
#   <out>/include/wasm32-wasip2/**.h
#   <out>/lib/wasm32-wasip2/{libc.a,librt.a,crt1-*.o,libclang_rt.builtins.a,...}
#   <out>/bin/wasm-component-ld
#
# Usage: ./create-bundle.sh [WASI_SDK_DIR] [OUT_DIR]
set -euo pipefail
...
rm -rf "$OUT"
mkdir -p "$OUT/include" "$OUT/lib/wasm32-wasip2" "$OUT/bin"
...
# The one host-specific binary (ships its own version-matched p1->p2 adapter).
cp "$SDK/bin/wasm-component-ld" "$OUT/bin/"

echo "bundle written to: $OUT"
du -sh "$OUT" "$OUT/include" "$OUT/lib" "$OUT/bin"
